...
@synthesize downBackView = _downBackView;
...
-(void)handleTapGesture:(UIGestureRecognizer*)gestureRecognizer
{
    ...
    if (CGRectContainsPoint(CGRectMake(0, 75*(YEAR_NUMBER+1), RIGHT_CONTERLLER_WIDTH, 88), locationInScrollView))
    {
        [self.delegate flowView:self didTapPageAtIndex:1000];
    }
}
...
- (void)initialize
{
    ...
    // 气球
    UIImageView *topImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, -122, RIGHT_CONTERLLER_WIDTH, 122)];
    [topImageView setImage:[UIImage imageNamed:@"top"]];
    [_scrollView addSubview:topImageView];
    
    UIImageView *lineImageView = [[UIImageView alloc] initWithFrame:CGRectMake(124.5, 0, 1, 75*(YEAR_NUMBER+1))];
    [lineImageView setImage:[UIImage imageNamed:@"listview_line"]];
    [_scrollView addSubview:lineImageView];
    
    _downBackView = [[UIView alloc] initWithFrame:CGRectMake(0, 75*(YEAR_NUMBER+1), RIGHT_CONTERLLER_WIDTH, 88)];
    _downBackView.backgroundColor = [UIColor clearColor];
    [_scrollView addSubview:_downBackView];
    
    UIImageView *birthImageView = [[UIImageView alloc] initWithFrame:CGRectMake(20, 27, 74, 23)];
    [birthImageView setImage:[UIImage imageNamed:@"year_0"]];
    [_downBackView addSubview:birthImageView];
    
    UIImageView *timeImageView = [[UIImageView alloc] initWithFrame:CGRectMake(22, 52, 12, 12)];
    [timeImageView setImage:[UIImage imageNamed:@"time"]];
    [_downBackView addSubview:timeImageView];
    
    _timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(34, 52, 60, 12)];
    _timeLabel.backgroundColor = [UIColor clearColor];
    _timeLabel.font = [UIFont systemFontOfSize:10];
    _timeLabel.textColor = [UIColor_Hex colorWithHex:0x57442e];
    _timeLabel.textAlignment = NSTextAlignmentCenter;
    [_downBackView addSubview:_timeLabel];
    
    //    down
    UIButton *calendarBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    calendarBtn.frame = CGRectMake(98.5, 0, 53, 88);
    [calendarBtn setImage:[UIImage imageNamed:@"down"] forState:UIControlStateNormal];
    [_downBackView addSubview:calendarBtn];
